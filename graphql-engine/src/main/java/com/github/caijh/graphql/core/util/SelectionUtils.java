...
public class SelectionUtils {
    ...
    private static DataFetcherData getDataFetcherData(DataFetchingEnvironment environment) {
        ...
        analyseGqlNode(environment, selectionSet, entityType, dataFetcherData, null, null, entityType instanceof GraphQLList, null);
    ...
    private static void analyseGqlNode(
        ...
        boolean currentNodeInList = inList || entityType instanceof GraphQLList;
        selectionSet.getFields()
                    .forEach(field -> analyseGqlNodeField(environment, entityType, dataFetcherData, parentTypePath, parentPath, field, currentNodeInList, bdf));
    }
    ...
    private static void analyseGqlNodeField(
        ...
        if (dataFetcher == null) {
            if (CollectionUtils.isEmpty(subFields)) {
                ...
                analyseGqlNode(environment, field
                        .getSelectionSet(), fieldGraphQLOutputType, dataFetcherData, currentTypePath, currentQueryPath, currentNodeInList, bdf);
            }
            return;
        }
        //添加DataFetcher内依赖的字段
        ...
        if (!(dataFetcher instanceof DataFetcherProxy)) {
            return;
        }
        ...
        if (!CollectionUtils.isEmpty(subFields) && batchDataFetcherData != null) {
            GraphQLOutputType fieldGraphQLOutputType = GraphqlTypeUtils.getFieldGraphQLOutputType(entityType, fieldName);
            analyseGqlNode(environment, field
                    .getSelectionSet(), fieldGraphQLOutputType, dataFetcherData, currentTypePath, currentQueryPath, false, batchDataFetcherData);
        }
    }
    ...
    private static void analyseGqlFieldDirective(DataFetchingEnvironment environment, DataFetcherData dataFetcherData, SelectedField selectedField, String parentTypePath, String parentQueryPath) {
        ...
        if (!directiveList.isEmpty()) {
            ...
            dataFetcherData
                    .addFieldDirectives(StringUtils.isEmpty(parentTypePath) ? fieldName : parentTypePath + GraphqlConsts.CHAR_DOT + fieldName, directiveList);
        }
    }
