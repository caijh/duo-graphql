...
public class GraphqlInvocationConfigure {
     ...
     * 当前引擎保存的图
     */
    private static final Map<String, GraphQL> graphQLMap = Maps.newConcurrentMap();
    ...
    public GraphQLInvocation getGraphQLInvocation() {
        return (invocationData, webRequest) -> {
            ...
            UserExecutionContext context = this.userExecutionContextFactory.get(invocationData, webRequest);
            this.setExecutionContextSchemaName(webRequest, context);
            ...
            if (this.executionMonitor != null) {
                this.executionMonitor.beforeInvocation(executionInput, webRequest);
            }
            ...
            executionResultCompletableFuture.thenRun(() -> {
                ...
                try {
                    requestData = this.objectMapper.writeValueAsString(invocationData);
                } catch (JsonProcessingException e) {
                    logger.error("发生错误！", e);
                }
                logger.info("graphql[{}]查询耗时 {} gql:{}", operationName, System.currentTimeMillis() - t1, requestData);
                if (this.executionMonitor != null) {
                    this.executionMonitor.afterInvocation(context, invocationData, webRequest);
                }
            });
    ...
    private void setExecutionContextSchemaName(WebRequest webRequest, UserExecutionContext context) {
        ...
        if (webRequest instanceof ServletWebRequest) {
            ...
            schemaName = this.graphqlProviderConfigure.getUrlSchemaName(url);
        } else {
