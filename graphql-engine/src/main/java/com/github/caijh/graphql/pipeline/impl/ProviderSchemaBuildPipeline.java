...
public class ProviderSchemaBuildPipeline implements Pipeline {
    ...
    public void doPipeline(RegistryState registryState) {
        registryState.getModuleContextMap().entrySet().forEach(
                entry -> this.buildProviderSchema(registryState, entry.getValue())
        );
    }
    ...
    private void buildProviderSchema(RegistryState state, GraphqlModuleContext moduleContext) {
        ...
        apiDto.getApis().forEach(api -> this.buildApiGraphQLFieldDefinition(state, moduleContext, api));
    }
    ...
    private void buildApiGraphQLFieldDefinition(RegistryState registryState, GraphqlModuleContext moduleContext, Api api) {
        ...
        if (api.getGraphqlJson() != null && api.getGraphqlJson()) {
            ...
            responseType = this.getGraphqlOutputType(registryState, moduleContext, response);
        ...
        if (!CollectionUtils.isEmpty(requestParams)) {
            requestParams.forEach(param -> {
                ...
                GraphQLInputType type = (GraphQLInputType) GraphqlTypeUtils
                        .getGraphqlInputType(registryState, moduleContext.getModuleName(), param.getEntityName());
                Assert.notNull(type, "typ不允许为空：" + param.getEntityName());
                boolean required = param.isRequired() || GraphqlTypeUtils.PATH_VARIABLE.equals(param.getAnnotation()) || GraphqlTypeUtils.REQUEST_BODY
                        .equals(param.getAnnotation());
                if (required) {
                    //必填的
    ...
    private GraphQLType getGraphqlOutputType(RegistryState registryState, GraphqlModuleContext moduleContext, EntityRef entityRef) {
        ...
        if (collection != null && collection) {
            // 列表
            return this.getGraphQLListType(registryState, moduleContext, entity);
        }
        ...
        return this.addAndGetOutputGraphQLType(registryState, moduleContext, entity);
    }
    ...
    private GraphQLType getGraphQLListType(RegistryState registryState, GraphqlModuleContext moduleContext, Entity entity) {
        ...
        GraphQLOutputType paramGraphQLOutputType = (GraphQLOutputType) this.getGraphqlOutputType(registryState, moduleContext, parameteredEntityRef);
    ...
    private GraphQLType addAndGetOutputGraphQLType(RegistryState registryState, GraphqlModuleContext moduleContext, Entity entity) {
        ...
        entity.getFields().forEach(field -> {
            ...
            GraphQLOutputType graphQLOutputType = this.buildGraphqlField(registryState, typeName, field);
            ...
            GraphQLOutputType fieldType = (GraphQLOutputType) this.getGraphqlOutputType(registryState, moduleContext, field);
            if (fieldType == null) {
                return;
            }
            ...
            //检查是否需要关联实体
            this.buildRefField(registryState, objectTypeBuilder, typeName, fieldName);
        });
    ...
    private void buildRefField(RegistryState registryState, GraphQLObjectType.Builder objectTypeBuilder, String typeName, String fieldName) {
        ...
        if (refDataFetcher instanceof DataFetcherProxy) {
            ...
            EntityRef response = api.getResponse();
            responseType = (GraphQLOutputType) this.getGraphqlOutputType(registryState, moduleContext, response);
            if (responseType == null) {
                logger.error("接口：{} {}(...)响应体异常", response.getEntityName(), api.getCode());
                return;
            }
        ...
        String newFieldName = this.newRefTypeName(refFieldName);
    ...
    private GraphQLOutputType buildGraphqlField(
        ...
        return (GraphQLOutputType) this.getGraphqlOutputType(registryState, moduleContext, api.getResponse());
    }
